...
```
python tools/project_refresh.py
```

Re-parses the netlist/BOM, lints every component card, runs `--open` plus reachability checks (when a card has `**Wiring targets:**`), and mechanically validates `docs/net-registry.md` against the netlist — one consolidated status instead of running several scripts separately. Doesn't call `datasheet_index.py`/`datasheet_quickstart.py` (those are per-new-datasheet tools, not part of a routine refresh) and doesn't write anything back anywhere — it's a status readout, not an update. Start here, then drill into whichever script's output actually needs a closer look.

## "I need to research a new component / datasheet"

```
python tools/datasheet_index.py "docs/Component Documentation/<PART>.pdf"
python tools/datasheet_quickstart.py "docs/Component Documentation/<PART>"
```
...
4. Write findings into `docs/components/<PART>.md` — copy [`docs/components/_TEMPLATE.md`](components/_TEMPLATE.md) as a starting point. Required header fields: `**Designator:**`, and `**Wiring targets:** <designator>/<part_name>` if any signals reach another IC (e.g. `U1/RK3576`). Follow `AP6275S.md` for the full shape: vendor pin table with Cradle Wiring + Status columns, Net name map once hierarchical ports exist, Known Gaps, External Validation with provenance tags.
5. Run `python tools/card_lint.py docs/components/<PART>.md` before considering the card done — catches the exact structural mistakes that have already broken the parser twice (compressed multi-pin rows with a combined name, "same as above"-style positional shorthand, missing Status values).

## "I need to check the current Altium state"

```
python tools/netlist_parse.py "docs/Altium/Netlist/Cradle.NET" "docs/Altium/BOM/Cradle.csv" "docs/Altium/Cradle"
```
...
```
python tools/registry_check.py docs/net-registry.md "docs/Altium/Netlist/Cradle.NET"
```

Mechanical only: for each row in `docs/net-registry.md`, reports whether that net name exists in the netlist and how many pins are on it. Does not interpret *why* a net is missing (stale export vs. not wired yet vs. unresolved hierarchical port). Also run automatically as part of `project_refresh.py`.
...
```
python tools/wiring_check.py docs/components/<PART>.md <designator> <target_designator> <target_name> "docs/Altium/Netlist/Cradle.NET"
```
...
```
python tools/wiring_check.py --open docs/components/<PART>.md <designator> "docs/Altium/Netlist/Cradle.NET"
```
...
|---|---|
| `docs/Component Documentation/` | Raw vendor PDF datasheets, plus generated `.index.md`/`.tables.md`/`.outline.md`/`.quickstart.md` per part |
| `docs/components/` | Curated component cards (`_TEMPLATE.md` to copy for new ones) |
| `docs/Altium/Netlist/`, `docs/Altium/BOM/` | Real, periodically re-exported Altium netlist + BOM — check freshness before trusting |
| `docs/Altium/sheet-map.md` | Sheet number → sheet name lookup |
| `docs/Altium/standard-parts.md` | User-maintained list of designators held as generic staged stock, not yet functionally placed |
| `docs/Altium/*.netlist-summary.md` | Generated output of `netlist_parse.py` |
| `docs/net-registry.md` | Cross-sheet net tracking (Layer 3) — seeded from Wireless, not yet covering every sheet, still hand-maintained (not auto-generated) |
| `tools/` | All the scripts referenced above |
| `docs/decisions-log.md`, `docs/architecture.md`, `docs/bom.md`, `docs/schematic-status.md` | Pre-existing project docs, unrelated to this tooling system, still the source of truth for locked decisions and overall board status |
