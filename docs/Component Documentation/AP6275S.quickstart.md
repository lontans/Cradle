...
Second pass over `datasheet_index.py`'s output, not a fresh PDF scan. Deterministic keyword triage only -- narrows where to look, doesn't explain why or what to do. Reading the flagged pins/pages and writing real integration guidance into `docs/components/<PART>.md` is still a human/Claude task, not something this script does.
