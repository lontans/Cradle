...
Designators the user holds/copy-pastes around the Altium canvas as generic stock — not yet assigned a specific circuit role. Maintained by hand; the BOM/netlist can't distinguish these from functionally-specific parts on their own (see `docs/co-design-workflow.md`, Layer 0.5 — several of these share exact part numbers with designators that *are* functionally placed, e.g. `C11` and `C84`/`C85` are the identical 10pF/0402/C0G part).
