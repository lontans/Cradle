...
Layer 3 from `docs/co-design-workflow.md`, seeded rather than left as "still proposed, not built." Scope: **only nets that cross a sheet boundary** (hierarchical ports). Pin-local facts (a pull resistor entirely within one sheet) belong in the relevant `docs/components/<PART>.md` instead — see that doc's Layer 1/Layer 3 split.
...
**Format not fully decided** (see co-design-workflow.md's open question — markdown here for consistency with the rest of `docs/`, may move to CSV if row count grows past what's comfortably skimmable).
