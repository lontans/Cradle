...
| 4 | Compute — Memory | Not started | LPDDR4X (K4UBE3D4AB-MGCL, U5), all decoupling. eMMC lives on Storage (row 6) — real BOM/netlist has `U3`/EMMC04G-M627-Y02U on `Cradle_Storage`, not here |
...
| 6 | Storage and SD | **In progress** | MicroSD slot (TF-01A, Card1): SD-mode wiring, CMD/DAT0-3 + mechanical card-detect pull-ups (30kOhm to STORAGE_3V3) done — see docs/decisions-log.md and cradle_sidecar/data/components/TF-01A.md. Open: mechanical CD pin's GPIO voltage domain unchecked (pending Compute-SoC). eMMC (EMMC04G-M627-Y02U, U3), all decoupling — moved here from Memory (see note in row 4) — not yet started, datasheet's ball map is a figure/diagram not machine-readable text |
