...
### 2026-07-09 — AP6275S CBUCK/ABUCK inductor finalized: 2.2uH, 1A/60mOhm (SLS3D16S2R2NTT)
**Decision:** Both internal buck bridging inductors use SLS3D16S2R2NTT — 2.2uH, 1A saturation current, 60mOhm DCR, 4x4mm shielded. Real Altium designators: `L10` bridges `ASR_VLX`(28)/`ABUCK_1P12`(29) (analog-core loop), `L11` bridges `CSR_VLX`(26)/`CBUCK_0P9`(25) (digital-core loop); `L9` is the same part staged/unplaced (see `standard-parts.md`). Output caps: `C89` (`ABUCK_1P12`-to-GND) and `C90` (`CBUCK_0P9`-to-GND), both 4.7uF X5R 0603 (CL10A475KP8NNNC); `C88` staged/unplaced.
**Why:** Closes the two open questions left by the 2026-07-07 entry above — inductance value (2.2uH vs 4.7uH, both evidence-backed from real shipped boards) and Isat rating. Took 2.2uH from the Geniatech OSM_BB/iMX8MM reference and the >=1A/<=80mOhm spec from the Geniatech CBD-3588 reference; neither AMPAK's datasheet nor either individual reference gives a first-principles derivation (switching frequency and per-rail load current for these internal bucks are undocumented — the closest datasheet data point is a whole-module "VBAT current consumption 1200mA(Peak)" figure, which bounds total module draw but doesn't break out per internal rail; see `AP6275S.md` Known Gaps, added 2026-07-09). The selected part's actual 60mOhm DCR beats the 80mOhm ceiling both references worked within.
Verification note: the netlist briefly showed `C89` wired to the `ASR_VLX` switch node instead of the `ABUCK_1P12` output (a deliberately-planted error to test the wiring-check workflow) — caught via netlist cross-reference, fixed in Altium, confirmed corrected in the 2026-07-09 00:34:47 re-export before this entry was written.

### 2026-07-09 — TF-01A microSD: SD-mode wiring, all bus pull-ups to STORAGE_3V3
**Decision:** `Card1` (TF-01A) wired SD-mode (not SPI): `CMD`/`DAT0`/`DAT1`/`DAT2`/`DAT3`-`CD` each get a 30kOhm pull-up (R45-R49) to `STORAGE_3V3`, matching the pull-up convention already used for Wireless-sheet SDIO. `CLK` has no pull-up (actively driven). The separate mechanical card-detect switch (pin 9, distinct from pin 2's electrical `DAT3`/`CD`) also gets a 30kOhm pull-up (R50) to `STORAGE_3V3`, into a to-be-assigned RK3576 GPIO.
**Why:** Standard SD bus pull-up practice, consistent with how the project already handled the Wireless sheet's SDIO lines. See `cradle_sidecar/data/components/TF-01A.md` for the full pin table and net name map.
**Open risk, not yet resolved:** pin 9's pull-up target (`STORAGE_3V3`, 3.3V) hasn't been checked against the voltage domain of whatever RK3576 GPIO bank ends up assigned to it — if that bank is 1.8V-only, this would overvoltage the pin. Not checkable until Compute-SoC does pin assignment; flagged in `TF-01A.md`'s Known Gaps so it isn't missed then.
