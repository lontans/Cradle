...
Comma-separate multiple targets if needed (e.g. `U1/RK3576, U9/RK3576`). Omit this line only if the part has no signals meant to reach another IC — `tools/project_refresh.py` uses it to run reachability checks.

**Vendor:** <vendor> **Datasheet:** `docs/Component Documentation/<PART>.pdf` **Structural index:** `docs/Component Documentation/<PART>.index.md` (or `.outline.md` for TRM-style docs).

Vendor facts use provenance tags per `docs/co-design-workflow.md`: `[VERIFIED — source]` / `[UNVERIFIED — source]`.
...
Add once hierarchical ports exist. Pin table keeps vendor pin names; this table maps them to real Altium net names. Sheet-local nets (clocks, RF, same-sheet loops) stay out of this table and out of `docs/net-registry.md` — see `AP6275S.md` for the pattern.
