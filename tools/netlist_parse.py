...
2. Programmatic access to per-designator pin coverage (which pin numbers on
   ...
   from a real pin table -- e.g. docs/components/<PART>.md -- which this
...
def freshness_line(*paths):
    ...
    docs/co-design-workflow.md, 2026-07-08 entry, for why this exists."""
    lines = []
    for p in paths:
...
def parse_bom(bom_path):
    ...
    part value -- see docs/Altium/sheet-map.md for why."""
    designator_map = {}
...
def load_sheet_map(path="docs/Altium/sheet-map.md"):
    p = Path(path)
    if not p.exists():
        return {}
    mapping = {}
...
def load_standard_parts(path="docs/Altium/standard-parts.md"):
...
def classify_floating_nets(nets, standard_parts):
    ...
    docs/co-design-workflow.md, 2026-07-08, for the fix."""
...
def write_summary(netlist_path, components, bom, nets, out_path, standard_parts=None, sheet_map=None):
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            ...
            f"parts (`docs/Altium/standard-parts.md` -- expected, excluded from review below), and "
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python netlist_parse.py <netlist.NET> <bom.csv> [output_prefix]")
        sys.exit(1)
    ...
    if any(row.get("sheet") for row in bom.values()):
        print(f"Sheet resolution: active (docs/Altium/sheet-map.md, {len(sheet_map)} sheets known).")
    else:
