...
def main():
    ...
    if Path(REGISTRY_PATH).exists():
        ...
        print(f"  -> run `python cradle_sidecar/tools/registry_check.py {REGISTRY_PATH} {NETLIST_PATH}` for full table")
    else:
    ...
    for card_path in card_paths:
        ...
        if lint_findings:
            print(f"  LINT: {len(lint_findings)} finding(s) -- run `python cradle_sidecar/tools/card_lint.py {card_str}` for detail")
        else:
            print("  LINT: clean")
        ...
        if open_results:
            ...
            print(f"    -> run `python cradle_sidecar/tools/wiring_check.py --open {card_str} {designator} {NETLIST_PATH}` for the full list")
        else:
        ...
        for target_designator, target_name in targets:
            ...
            if unwired or not_reaching:
                print(
                    f"    -> run `python cradle_sidecar/tools/wiring_check.py {card_str} {designator} "
