...
- `[MANUAL — user, <date>]` — added 2026-07-09, for a fact the user supplies directly, not transcribed from any document. This is a different axis than VERIFIED/UNVERIFIED, not a lower rung on the same ladder: UNVERIFIED means a document exists but hasn't been independently confirmed, while MANUAL means there is deliberately no document backing the fact (datasheet missing, scrambled, or silent on the point) and the user's own statement *is* the source. Motivated by a recurring real problem: several vendor datasheets in this project are bad enough (no application circuit, scrambled text layer, missing values — see AP6275S's Known Gaps for three examples) that the only path to a usable card is the user filling the gap from their own knowledge. Treat a MANUAL fact as confident, not tentative — it just isn't independently checkable the way a datasheet citation is. If a datasheet reference later turns up for the same fact, replace the tag rather than stacking both.

Every component card also now carries a card-level **Card origin:** field (`Datasheet-derived` / `Manually authored` / `Mixed`) in its header, so a reader (or the sidecar UI's card list) can tell at a glance which cards leaned on this path without reading every tag. See `_TEMPLATE.md`.

**Correction (2026-07-09), same day as the tag was added:** the first version of the sidecar's manual-card path put a blank `_TEMPLATE.md`-shaped textarea in front of the user and called it "the manual-authoring workflow" — user feedback, correctly, was that this is real friction: hand-writing the whole card shape (provenance tags, pin-table markdown, every header field) from nothing is exactly the same datasheet-reading work an agent already does for every other card, just handed to the human instead. Reframed: **agent drafts, user reviews and fills the genuine gaps** — not "agent extracts cleanly or the user does everything by hand" as the only two options. Concretely: when `datasheet_index.py`'s mechanical PyMuPDF pass comes back empty (scanned image, scrambled text layer — exactly TF-01A's and part of AP6275S's situation), the agent reads the datasheet directly via vision (`Read` on the PDF, looking at rendered pages, not text-layer parsing) plus a web search for the part, drafts the card with real `VERIFIED`/`UNVERIFIED` tags for whatever it could determine, and leaves everything else as an explicit unfilled placeholder or Known-Gaps/Open-questions entry rather than guessing. The user's job shrinks to reviewing that draft and filling the real gaps with `MANUAL`-tagged facts, using the sidecar editor's new auto-lint-on-open and **Next blank** button to jump straight to what's missing instead of reading the whole file. `card_lint.py` gained a mechanical placeholder check (`<...>` tokens left over from the template) specifically so a half-drafted card can't silently pass as finished. Blank-template authoring is still available, but now scoped to its actually-correct case: a part with no datasheet to draft from at all, not the general path.

Also resolved explicitly, since it came up while reframing this: the sidecar server itself still does not invoke AI (no LLM call, no API key, no added cost/latency baked into a small local tool) — that boundary from the original sidecar design didn't change. The "agent drafts" step happens in a Claude Code conversation, same as every other datasheet-reading task in this project; the sidecar's role is unchanged in kind (review, lint, save), just now aimed at reviewing an agent-authored draft instead of a blank one.

**Second correction, same day:** even reviewing a drafted card in a raw-markdown textarea was still, per direct user feedback, "pretty poor for reading" — the reviewer had to hold the actual datasheet PDF in a separate window while cross-checking the card's prose against it, and any small correction meant opening the full editor and finding the right spot in a wall of markdown. Two changes:

1. **The datasheet PDF is now embedded directly in the card view** (`GET /datasheets/<name>`, streamed inline; the sidecar reads which PDF from the card's own `**Datasheet:**` header via `data_api.extract_datasheet_name()` — self-contained, works the same whether the card came from a BOM-matched sheet part or the standalone Cards list, unlike the existing `find_datasheet_assets()` which needs a BOM part-name match). Card markdown renders beside it, not in a separate tab.
2. **Provenance citations became live navigation, not just colored text**: a `[VERIFIED — ... p.12]` tag (already colorized from the 2026-07-09 status/provenance-highlighting pass) is now clickable when a PDF pane is present, and jumps the embedded viewer to that page — checking a cited claim against its actual source is one click. This is the one deliberate "signature" interaction for this rework (per a frontend-design pass): it's the citation *doing* the navigation, not a decorative flourish next to it.
3. **`cradle_sidecar/tools/card_note.py`** (new) is the deterministic write-back half of "add one finding without opening the full editor" — same philosophy as `netlist_parse.py`/`registry_check.py`/`card_lint.py`: an agent (or the UI's new **Add a note** box, which calls the identical `data_api.add_card_note()` path) supplies already-reasoned text plus a `VERIFIED`/`UNVERIFIED`/`MANUAL` tag, and the script guarantees the on-disk formatting (timestamp, section placement, tag syntax) is identical regardless of who called it. Appends to a `## AI reading notes` section, created on first use. The full raw-markdown editor (`Edit`) still exists and is still the right tool for restructuring — new pin-table rows, rewriting a section — the note path is specifically for the common case that doesn't need that: "I checked this claim, here's what I found."

Net effect: the editor from the first correction didn't go away, but it's no longer the primary review surface — reading the card next to its actual source, and appending single tagged findings, is now the default; the full editor is reserved for structural edits.
...
**Bug found and fixed (2026-07-09): net-registry.md asserted a single direction for genuinely bidirectional nets.** User reported the SDIO signals (`SDCMD`/`SDCLK`/`SD0`-`SD3`) and the two general-purpose GPIOs (`WL_GPIO_10`/`11`) showed as one-directional in the registry despite the AP6275S datasheet marking all of them `I/O` — correctly rendered as `I/O` in the component card's own pin table (never wrong), only wrong in `net-registry.md`'s `Driven from`/`Consumed by` columns.

**Root cause:** those two columns were designed assuming every net has a single driver and a single consumer — true for point-to-point signals (`WL_REG_ON`, UART `TXD`/`RXD`, `HOST_WAKE` lines — checked all of these against the card's Type column while fixing this, all genuinely unidirectional, all correct in the registry). False for a shared bus: SDIO `CMD`/`DATA` lines are physically bidirectional by protocol (host drives `CMD` to issue a command, device drives `CMD` back with the response; `DATA` runs host->device on writes, device->host on reads). The registry rows conflated "which side initiates/controls the transaction" (the host, always) with "which side electrically drives the wire" (both) — a real semantic error, not a typo, and the same error repeated for the GPIOs (asserted "driven from Wireless" for I/O-capable pins whose target isn't even decided yet).

**Fix:** for the 8 affected rows, both `Driven from` and `Consumed by` now read `Compute-SoC <-> Wireless (bidirectional)` (or `<-> TBD` for the GPIOs, since their target is still undecided) instead of picking one side. No schema change — stayed inside the existing 8-column format rather than adding a new column, since `registry_check.py`'s row parser is column-count-sensitive and a new column would have needed changes there too for a fix that didn't require it. Verified `registry_check.py` still parses all 19 rows post-fix (unchanged: 19 present, 0 missing), and confirmed the sidecar UI's `sheet_contracts()` — which computes direction by substring-matching the sheet's aliases against both columns independently — now correctly resolves these as `['out', 'in']` (renders as `<->` both) instead of one-sided, live-tested via the running server before and after.

**Why this is worth generalizing, not just patching the 8 rows:** the same one-driver-one-consumer assumption will be wrong again the next time a shared bus (I2C, another SDIO-like interface) gets added to the registry. The fix here is a naming convention (`A <-> B (bidirectional)` in both cells), not a structural guarantee — nothing currently stops a future row from being written the same wrong way. Worth a `card_lint.py`-style check someday: flag any registry row whose net name's source pin has Type `I/O` in its component card but whose registry `Driven from`/`Consumed by` don't both mention the same two parties.
